  ...
  useEffect(() => {
    const fetchBestsellers = async () => {
      try {
        ...
        if (data.length > 0) {
          // Dynamic items mapped from database
    ...
    <main className="bg-[#0B0F17] text-slate-100 h-auto pb-2">
      ...
      <section className="bg-[#111827] border-y border-slate-800 py-3 px-4">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-2 text-center text-xs">
          <div className="p-1">
            <span className="text-[#38BDF8] font-black text-base block">
              10K+
            </span>
            <span className="text-slate-400 text-[10px]">Happy Customers</span>
          </div>
          <div className="p-1">
            <span className="text-[#38BDF8] font-black text-base block">
              4.9 ★
            </span>
            <span className="text-slate-400 text-[10px]">Top Rated Beans</span>
          </div>
          <div className="p-1">
            <span className="text-[#38BDF8] font-black text-base block">
              12 Mins
            </span>
            <span className="text-slate-400 text-[10px]">Avg Prep Time</span>
          </div>
          <div className="p-1">
            <span className="text-[#38BDF8] font-black text-base block">
              100%
            </span>
            <span className="text-slate-400 text-[10px]">Fresh Roasted</span>
          </div>
        </div>
      </section>
      ...
      <section className="pt-6 pb-4 px-4 max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-3">
          <div>
            <span className="text-[#38BDF8] text-[10px] font-bold uppercase tracking-wider block">
              Explore Menu
            </span>
          ...
          <Link
            to="/menu"
            className="text-[#38BDF8] text-xs font-bold hover:underline"
          >
            View All →
          </Link>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <Link
              key={i}
              to="/menu"
              className="bg-[#111827] border border-slate-800 hover:border-[#38BDF8]/50 p-3.5 rounded-2xl transition group"
            >
              <p className="text-xs font-bold text-white group-hover:text-[#38BDF8]">
                {cat.title}
              </p>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className="bg-[#111827] border border-slate-800/80 rounded-2xl p-3 h-48 animate-pulse"
              />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {specialItems.map((item) => (
              <div
                key={item.id}
                className="bg-[#111827] border border-slate-800/80 rounded-2xl p-3 flex flex-col justify-between shadow-lg min-h-[220px]"
              >
                <div>
